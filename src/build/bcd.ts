...
const forceKeepAlive: Record<string, string[]> = {
  ...
  VREyeParameters: ["fieldOfView", "offset", "renderHeight", "renderWidth"],
...
export function getRemovalDataFromBcd(webidl: Browser.WebIdl): Browser.WebIdl {
  ...
  for (const [key, value] of Object.entries(bcd.api)) {
    ...
    const removalItem: Record<string, any> = {};
    if (!isEmpty(methods)) {
