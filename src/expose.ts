...
export function getExposedTypes(
  ...
  if (webidl.interfaces) {
    filtered.interfaces!.interface = filter(webidl.interfaces.interface, (o) =>
      exposesTo(o, target)
    );
  ...
  const isKnownName = (o: { name: string }) =>
    knownIDLTypes.has(o.name) || forceKnownTypes.has(o.name);

  if (webidl.typedefs) {
    const referenced = webidl.typedefs.typedef.filter(
      (t) =>
        knownIDLTypes.has(t["new-type"]) || forceKnownTypes.has(t["new-type"])
    );
  ...
  if (webidl["callback-functions"])
    filtered["callback-functions"]!["callback-function"] = filterProperties(
      webidl["callback-functions"]!["callback-function"],
      isKnownName
    );
  if (webidl["callback-interfaces"])
    filtered["callback-interfaces"]!.interface = filterProperties(
      webidl["callback-interfaces"]!.interface,
      isKnownName
    );
